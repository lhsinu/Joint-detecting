...
public:
    ...
    bool setup(const uint8_t addr,int mode,const MPU9250Setting& mpu_setting = MPU9250Setting(), WireType& w = Wire) {

        if (mode==0) Wire.begin(21, 22, 400000);
        else if (mode==1) Wire.begin(7, 33, 400000);
        else if (mode==2) Wire.begin(13, 33, 400000);
        else if (mode==3) Wire.begin(14, 33, 400000);
        else if (mode==4) Wire.begin(32, 33, 400000);
        ...
        if (isConnectedMPU9250()) {
            initMPU9250();
//            if (isConnectedAK8963())              //////// MPU6050 에는 AK8963가 없어서 삭제하는 주석을 추가함
//                initAK8963(); 
//            else {
//                if (b_verbose){
//                     delay(3000);
...
//            }
    ...
    bool update(int mode) {
        ...
        update_accel_gyro();
//        update_mag();             ///// 지자계 값은 필요없어서 삭제하는 주석을 추가함
